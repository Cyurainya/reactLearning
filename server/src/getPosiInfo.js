module.exports = async (ctx) => {
  ...
  if (key == "南极洲测试") {
    ...
    ctx.body = JSON.stringify({
      code: 0,
      message: "success",
      data: JSON.parse(mockDataZhaoke),
    });
    return;
  } else if (key == "南极洲宾馆") {
    console.log(key);
    ctx.body = JSON.stringify({
      code: 0,
      message: "success",
      data: JSON.parse(mockBinguan),
    });
    return;
  }
  //所以只有key是南极洲的时候才返回所有信息
  ctx.body = JSON.stringify({
    code: 0,
    message: "success",
    data: [],
